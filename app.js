 ...
 app.get('/refresh_members', async function(req, res) {
     ...
     if (await checkGeneratingFlag(results['access_code'])) {
         console.log("generating flag is true and about to render loading");
         //  res.render('loading.html', { results: JSON.stringify(results) });
         res.send(["next_page", results]);
     } else {
         res.send(["stay", results]);
     }
 });
 ...
 app.get('/generate', async function(req, res) {
     console.log(req.method + " " + req.route.path);
     console.log("about to check generating flag in /generate");
     if (!await checkGeneratingFlag(results['access_code'])) {
         const dbRef = ref(database);
         ...
         let playlist = await generatePlaylist(results['access_code'], results['group_name'], req.query.length);
         if (playlist) {
             res.send(playlist)
             const dbRef = ref(database);
             await update(child(dbRef, results['access_code']), {
     ...
     while (playlist_id == "invalid") {
         playlist_id = await getGeneratedPlaylist(results['access_code']);
     }
     playlist_id = await getGeneratedPlaylist(results['access_code']);
