...
async function CheckLength() {
    ...
    for (let i = 0; i < IDs.length / 100; i++) {
        ...
        for (let j = 0; j < song_feature_list.length; j++) {
            ...
            playlist_array.push(song_feature_list[j].id);
            id_to_time.set(song_feature_list[j].id, song_feature_list[j].duration_ms);
        }
    }
    console.log("ID TO TIME:", id_to_time);
    ...
    if (long_enough) {
        ...
        let temp_time = 0;
        let index = 0;
        console.log("LENGTH:", playlist_length);
        while (temp_time < playlist_length) {
            console.log("TEMP:", temp_time);
            temp_time += id_to_time.get(playlist_array[index]);
            index++;
            playlist_set.add(playlist_array[index]);
        }
    }
    return long_enough;
}
...
async function MainLoop() {
    ...
    for (let i = 0; i < num_users; i++) {
        if (i != 1) {
            await followPlaylist(code, users[i], playlist);
        }
    }
    console.log("DONE");
    return playlist;
